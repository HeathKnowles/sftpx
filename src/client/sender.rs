// Client-side file sending logic
...
/// Handles sending files from client to server using chunked transfer
...
impl DataSender {
    ...
    /// 
    /// # Arguments
    /// * `connection` - The client connection to send data on
    ...
    pub fn send_file(
        ...
        log::info!(
            "Client: Starting file upload: {} ({} bytes, {} chunks)",
            file_path.display(),
            chunker.file_size(),
            total_chunks
        );
        ...
        while let Some(chunk_packet) = chunker.next_chunk()? {
            ...
            match connection.stream_send(stream_id, &chunk_packet, is_last) {
                Ok(written) => {
                    ...
                    log::debug!(
                        "Client: Sent chunk {}/{}: {} bytes (progress: {:.1}%)",
                        chunk_count,
                        total_chunks,
                        written,
                ...
                Err(e) => {
                    log::error!("Client: Failed to send chunk {}: {:?}", chunk_count, e);
                    return Err(e.into());
                }
            }
        }
        ...
        log::info!(
            "Client: File upload complete: {} bytes in {} chunks",
            bytes_sent,
            chunk_count
        );
    ...
    pub fn send_data(
        ...
        let written = connection.stream_send(stream_id, data, fin)?;
        self.total_bytes_sent += written as u64;
        
        log::debug!(
            "Client: Sent {} bytes on stream {} (fin: {})",
            written,
            stream_id,
            fin
        );
...
mod tests {
    ...
    #[test]
    fn test_data_sender_default() {
        let sender = DataSender::default();
        assert_eq!(sender.total_bytes_sent(), 0);
    }
